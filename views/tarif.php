<table id="show" border="1" cellspacing="0" cellpadding="5">
	<tr>
		...
		<th width="50">No</th>
		<th>
			<form action="<?php echo $config->base_url();?>tarif.html" method="post">
		...
		<th>
			<form action="<?php echo $config->base_url();?>tarif.html" method="post">
				...
				<input type="hidden" name="order" value="<?php echo $order=="desc"?"asc":"desc";?>" />
				<input type="hidden" name="like" value="<?php echo $like;?>" />
				<input type="submit" value="Nama Frenchise" />
			</form>
		</th>
		<th>
			<form action="<?php echo $config->base_url();?>tarif.html" method="post">
	...
	<tr <?php if($zebra%2==1){echo "class='normal zebra'";}else{echo "class='normal'";}?>>
		<td align=center valign="top"><a href="<?php echo $config->base_url();?>select_tarif/<?php echo $tampil['id_tarif'];?>.html" class="ubah"><img src="<?php echo $config->base_url();?>images/edit.png" class="tools"></a></td>
		...
		<a href="<?php echo $config->base_url();?>delete_tarif/<?php echo $tampil['id_tarif'];?>.html" onclick="return confirm('Delete <?php echo $tampil['nama_setting'];?>?');"><img src="<?php echo $config->base_url();?>images/delete.png" class="tools"></a>
		...
		<tr>
			...
			<td>
				...
				<select name="field">
					...
					<option value="nama" <?php if($field=="nama"){echo "selected";}?>>Nama Frenchise</option>
